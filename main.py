import tkinter as tk

# Create window
root = tk.Tk()
root.title("Simple Calculator")
root.geometry("300x400")

# Input field
entry = tk.Entry(root, width=20, font=("Arial", 20), borderwidth=5)
entry.grid(row=0, column=0, columnspan=4, padx=10, pady=10)


# Function to add numbers/operators to screen
def click(number):
    current = entry.get()
    entry.delete(0, tk.END)
    entry.insert(0, str(current) + str(number))


# Function to clear screen
def clear():
    entry.delete(0, tk.END)


# Function to calculate result
def equal():
    result = eval(entry.get())

    entry.delete(0, tk.END)
    entry.insert(0, result)


# Buttons
button_1 = tk.Button(root, text="1", padx=20, pady=20,
                     command=lambda: click(1))
button_2 = tk.Button(root, text="2", padx=20, pady=20,
                     command=lambda: click(2))
button_3 = tk.Button(root, text="3", padx=20, pady=20,
                     command=lambda: click(3))

button_4 = tk.Button(root, text="4", padx=20, pady=20,
                     command=lambda: click(4))
button_5 = tk.Button(root, text="5", padx=20, pady=20,
                     command=lambda: click(5))
button_6 = tk.Button(root, text="6", padx=20, pady=20,
                     command=lambda: click(6))

button_7 = tk.Button(root, text="7", padx=20, pady=20,
                     command=lambda: click(7))
button_8 = tk.Button(root, text="8", padx=20, pady=20,
                     command=lambda: click(8))
button_9 = tk.Button(root, text="9", padx=20, pady=20,
                     command=lambda: click(9))

button_0 = tk.Button(root, text="0", padx=20, pady=20,
                     command=lambda: click(0))

# Operator buttons
button_add = tk.Button(root, text="+", padx=20, pady=20,
                       command=lambda: click("+"))

button_sub = tk.Button(root, text="-", padx=20, pady=20,
                       command=lambda: click("-"))

button_mul = tk.Button(root, text="*", padx=20, pady=20,
                       command=lambda: click("*"))

button_div = tk.Button(root, text="/", padx=20, pady=20,
                       command=lambda: click("/"))

button_equal = tk.Button(root, text="=", padx=20, pady=20,
                         command=equal)

button_clear = tk.Button(root, text="C", padx=20, pady=20,
                         command=clear)

# Place buttons using grid()

button_7.grid(row=1, column=0)
button_8.grid(row=1, column=1)
button_9.grid(row=1, column=2)
button_div.grid(row=1, column=3)

button_4.grid(row=2, column=0)
button_5.grid(row=2, column=1)
button_6.grid(row=2, column=2)
button_mul.grid(row=2, column=3)

button_1.grid(row=3, column=0)
button_2.grid(row=3, column=1)
button_3.grid(row=3, column=2)
button_sub.grid(row=3, column=3)

button_clear.grid(row=4, column=0)
button_0.grid(row=4, column=1)
button_equal.grid(row=4, column=2)
button_add.grid(row=4, column=3)

# Run app
root.mainloop()